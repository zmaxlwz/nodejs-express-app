...
var add_labeled_sample = function(id, label) {
	...
	labeled_samples[id] = received_samples[id].slice(3).concat([label]);
	all_labeled_samples_sofar[id] = received_samples[id].slice(3).concat([label]);
...
var mouseClick = function() {
	...
	if(circle_stroke_width === 2){
		...
		if(circle.attr("stroke") === "Green"){  //should be Green
			...
			//decrease 1 to the list value for labeled points
		...
		if ($('#point-label').val() === 'positive'){
			...
			//increase 1 to the list value for labeled points
			$("#list_num_labeled").text( +$("#list_num_labeled").text() + 1);
...
function zoom_in_plot(svg, x_range, y_range){
	//after user clicks zoom-in, replot the chart
	//svg: the svg to plot  
	//x_range:  {attr_name, min_value, max_value}
	//y_range:  {attr_name, min_value, max_value}
	...
	for (var key in all_labeled_samples_sofar) {
	  if (all_labeled_samples_sofar.hasOwnProperty(key)) {
		  var sample_x_value = all_labeled_samples_sofar[key][x_index_to_plot];
		  var sample_y_value = all_labeled_samples_sofar[key][y_index_to_plot];
		  
		  if(sample_x_value >= x_minvalue && sample_x_value <= x_maxvalue && sample_y_value >= y_minvalue && sample_y_value <= y_maxvalue){
			  //check if the sample is within the display range
			  plot_data.push(
				  {
					  id: 0,
					  key_id: key,
	...
	for (var key in received_samples) {
	  if (received_samples.hasOwnProperty(key) && !(key in all_labeled_samples_sofar)) {
		  var sample_x_value = received_samples[key][x_index_to_plot + 3];
		  var sample_y_value = received_samples[key][y_index_to_plot + 3];
...
function change_visualization_space(svg, new_x_index, new_y_index){
	...
	var x_name = current_x_range.attr_name;
	var x_minvalue = current_x_range.min_value;
	var x_maxvalue = current_x_range.max_value;
	var y_name = current_y_range.attr_name;
	var y_minvalue = current_y_range.min_value;
	var y_maxvalue = current_y_range.max_value;
...
function plot_more_samples(data, svg, x_range, y_range){
	...
	d3.selectAll(".circles").each(function(d, i){ 
		if(d3.select(this).attr("stroke-width") == 0){
			d3.select(this).style("fill", "Gray").style("opacity", 0.25);
		} 
...
var change_exploration_interface = function(){
	...
	current_x_range = {attr_name: all_attr_info[x_index_to_plot].attr_name, min_value: +all_attr_info[x_index_to_plot].min_value, max_value: +all_attr_info[x_index_to_plot].max_value};
	current_y_range = {attr_name: all_attr_info[y_index_to_plot].attr_name, min_value: +all_attr_info[y_index_to_plot].min_value, max_value: +all_attr_info[y_index_to_plot].max_value};
	...
	if(truth_query_shape != null && truth_query_data.length > 0){
		...
		plot_true_query_shape(d3.select('#chart svg'), truth_query_data, truth_query_shape, current_x_range, current_y_range);
	...
	if(model_learned === true && all_attr_info.length <= 2){
		...
		for(var i = 0; i<grid_points.length; i++){
			...
			var grid_x_value = grid_points[i].data_value[x_index_to_plot];
			var grid_y_value = grid_points[i].data_value[y_index_to_plot];
			
			if(grid_x_value >= current_x_range.min_value && grid_x_value <= current_x_range.max_value && grid_y_value >= current_y_range.min_value && grid_y_value <= current_y_range.max_value){
		...
		grid_points_plot(grid_point_data_to_plot, d3.select('#chart svg'), current_x_range, current_y_range);
...
var scatterplot_init = function(){
	...
	*/
	...
	$("#to-label-btn").on('click', function(event){
		...
		//initialize the number of positve and negative labeled samples
		$("#list_num_positive").text(0);
		$("#list_num_negative").text(0);
		$("#list_num_labeled").text(0);
	...
	$("#zoom-in-btn").on('click', function(event){
		//console.log($("#zoomin-x-minvalue").val());
		//console.log($("#zoomin-y-minvalue").val());
		
		//zoom in the scatter plot chart and replot the chart
		
		var zoomin_x_minvalue = $("#zoomin-x-minvalue").val();
		var zoomin_x_maxvalue = $("#zoomin-x-maxvalue").val();
		var zoomin_y_minvalue = $("#zoomin-y-minvalue").val();
		var zoomin_y_maxvalue = $("#zoomin-y-maxvalue").val();
		
		
		var x_index_to_plot = $("#x-attr-name").prop("selectedIndex");
		var y_index_to_plot = $("#y-attr-name").prop("selectedIndex");
		
		if(zoomin_x_minvalue == ""){
			zoomin_x_minvalue = all_attr_info[x_index_to_plot].min_value;
		}
		if(zoomin_x_maxvalue == ""){
			zoomin_x_maxvalue = all_attr_info[x_index_to_plot].max_value;
		}
		if(zoomin_y_minvalue == ""){
			zoomin_y_minvalue = all_attr_info[y_index_to_plot].min_value;
		}
		if(zoomin_y_maxvalue == ""){
			zoomin_y_maxvalue = all_attr_info[y_index_to_plot].max_value;
		}
		
		/*
		if(zoomin_x_minvalue == "" || zoomin_x_maxvalue == "" || zoomin_y_minvalue == "" || zoomin_y_maxvalue == ""){
			...
			$(this).popover({
				...
				trigger:'click',
				//title: "Input Error",
				content: "All input should be non-empty." 
			});
		...
		*/
		...
		if(isNaN(zoomin_x_minvalue) || isNaN(zoomin_x_maxvalue) || isNaN(zoomin_y_minvalue) || isNaN(zoomin_y_maxvalue)){
			$(this).popover("destroy");
			...
			$(this).popover({
				...
				trigger:'click',
				//title: "Input Error",
				content: "All input should be numbers." 
			});
	
			$(this).popover("show");
	
		}
		else if(+zoomin_x_minvalue >= +zoomin_x_maxvalue || +zoomin_y_minvalue >= +zoomin_y_maxvalue){
			$(this).popover("destroy");
			...
			$(this).popover({
				...
				trigger:'click',
				//title: "Input Error",
				content: "The Min value should be less than Max value." 
			});
			...
			current_x_range = {attr_name: $("#x-attr-name").val(), min_value: +zoomin_x_minvalue, max_value: +zoomin_x_maxvalue};
			current_y_range = {attr_name: $("#y-attr-name").val(), min_value: +zoomin_y_minvalue, max_value: +zoomin_y_maxvalue};
			
			//plot samples in the new range	
			zoom_in_plot(d3.select('#chart svg'), current_x_range, current_y_range);
	...
	$("#goback-btn").on('click', function(event){
		//console.log($("#zoomin-x-maxvalue").val());
		//console.log($("#zoomin-y-maxvalue").val());
		
		$("#zoomin-x-minvalue").val("");
		$("#zoomin-x-maxvalue").val("");
		$("#zoomin-y-minvalue").val("");
		$("#zoomin-y-maxvalue").val("");
		...
		//change the current_x_range and current_y_range
		current_x_range = {attr_name: all_attr_info[x_index_to_plot].attr_name, min_value: +all_attr_info[x_index_to_plot].min_value, max_value: +all_attr_info[x_index_to_plot].max_value};
		current_y_range = {attr_name: all_attr_info[y_index_to_plot].attr_name, min_value: +all_attr_info[y_index_to_plot].min_value, max_value: +all_attr_info[y_index_to_plot].max_value};
		
		//recover the chart to the original range and replot
		//plot samples in the new range	
		zoom_in_plot(d3.select('#chart svg'), current_x_range, current_y_range);
	...
	$("#explore-start-btn").on('click', function(event){
		...
		*/
        ...
        d3.json("start_exploration.php", function(error, data) {
			...
			//initialize the number of labeled samples to be 0
			$("#list_num_labeled").text(0);
			...
			//current_x_range and current_y_range are global variables
			current_x_range = {attr_name: all_attr_info[attr_x_index_to_plot].attr_name, min_value: +all_attr_info[attr_x_index_to_plot].min_value, max_value: +all_attr_info[attr_x_index_to_plot].max_value};
			current_y_range = {attr_name: all_attr_info[attr_y_index_to_plot].attr_name, min_value: +all_attr_info[attr_y_index_to_plot].min_value, max_value: +all_attr_info[attr_y_index_to_plot].max_value};
				...
				sample_data.push(
					{
						id: sample_id,
						key_id: sample_key,
						visualattr1_value: data.samples[i][1],
						visualattr2_value: data.samples[i][2],						
						x_value: data.samples[i][attr_x_index_to_plot + 3],
					 	y_value: data.samples[i][attr_y_index_to_plot + 3]
					}					
				);
			...
			scatter_plot(sample_data, d3.select('#chart svg'), current_x_range, current_y_range);
			
			//console.log(data);
			//console.log(data.truth);
			//console.log(data.truth_shape);
			/*
			//var truth_query_shape = null;
			...
			*/
			...
			if(data.truth_shape != undefined && ($("#scenario_num_input").val() == "Scenario 4" || $("#scenario_num_input").val() == "Scenario 5")){
				truth_query_shape = data.truth_shape;
			}
	...
	$("#next-iteration-btn").on('click', function(event){
        ...
        d3.json("next_exploration.php", function(error, data) {
			...
			//console.log(+$("#list_num_samples").text());
			...
			for(var i = 0; i<sample_num; i++){
				...
				var sample_x_value = data.samples[i][attr_x_index_to_plot + 3];
				var sample_y_value = data.samples[i][attr_y_index_to_plot + 3];
				
				if(sample_x_value >= current_x_range.min_value && sample_x_value <= current_x_range.max_value && sample_y_value >= current_y_range.min_value && sample_y_value <= current_y_range.max_value){
					sample_data.push(
						{
							id: sample_id,
							key_id: sample_key,						
			...
			plot_more_samples(sample_data, d3.select('#chart svg'), current_x_range, current_y_range);
			...
			if(all_attr_info.length <= 2){
				...
				for(var i = 0; i<grid_points_num; i++){
					...
					var grid_x_value = grid_points[i].data_value[attr_x_index_to_plot];
					var grid_y_value = grid_points[i].data_value[attr_y_index_to_plot];
