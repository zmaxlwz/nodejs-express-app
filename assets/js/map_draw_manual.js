    ...
    for (i = 0; i < data.length; i++) {
        ...
        google.maps.event.addListener(marker, 'mouseover', (function(marker, i) {
            return function() {
                ...
                infowindow.setContent(html);
                infowindow.open(map, marker);
                if (global_reviewed_objects.indexOf(data[i].id.toString()) == -1) {
                	global_reviewed_objects.push(data[i].id.toString());
                }
                if (local_reviewed_objects.indexOf(data[i].id.toString()) == -1) {
                	local_reviewed_objects.push(data[i].id.toString());
                }
