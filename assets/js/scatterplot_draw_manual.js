...
var header = ["picture", "price", "beds", "baths", "size", "town", "crime", "population", "education", "income", "review"];
var global_x_range;
var global_y_range;
function btnClicked(elmt, id) {
      elmt.style.background = 'yellow';
      if (global_reviewed_objects.indexOf(id.toString()) == -1) {
        global_reviewed_objects.push(id.toString());
      }
      if (local_reviewed_objects.indexOf(id.toString()) == -1) {
        local_reviewed_objects.push(id.toString());
      }
      $("#list_reviewed_objects").html(global_reviewed_objects.length);
      $("#list_num_samples").html(local_reviewed_objects.length);
...
// 	  "<br/></p>";
  ...
  var mouseClick = function() {
    ...
    //if (circle.attr("overlap") != -1) {
        ...
        document.getElementById('tableContainer').setAttribute("style","width:1000px");
        ...
        for (i = 0; i < tmpData.length; i++) {
            ...
            var color = "white";
            if (local_reviewed_objects.indexOf(tmpData[i].id.toString()) > -1) {
              color = "yellow";
            }
            result += "<td> <button style='width:67px;background:" + color + "' id='review_btn_"+ tmpData[i].id+"' onClick='btnClicked(this,"+tmpData[i].id+")'>review</button></td></tr>";
        }
        result += "</tbody></table>";
