...
var old_sample_current_labeled = [];
...
function btnClicked(elmt, id, circle) {
    ...
    if (elmt.style.background === 'white') {
        ...
        if (elmtid.indexOf('positive') > -1) {
          if (new_samples.indexOf(id.toString()) == -1 && document.getElementById('negative_btn_'+id).style.background == 'yellow') {
            if (old_sample_current_labeled.indexOf(id.toString()) == -1) {
              return;
            }
          }
          ...
          if (new_samples.indexOf(id.toString()) == -1 && document.getElementById('positive_btn_'+id).style.background == 'yellow') {
            if (old_sample_current_labeled.indexOf(id.toString()) == -1) {
              return;
            }
          }
        ...
        if (new_samples.indexOf(id.toString()) == -1 && old_sample_current_labeled.indexOf(id.toString())) {
          old_sample_current_labeled.push(id.toString());
        }
        elmt.style.background = 'yellow';
    }
    else {
      if (new_samples.indexOf(id.toString()) == -1 && old_sample_current_labeled.indexOf(id.toString()) == -1) {
        return;
      }
...
function kemi_scatter_plot(data, svg, x_range, y_range, filter){
...
//y_range:  {attr_name, min_value, max_value}
  ...
  if (filter) {
    for (var i = 0; i < data.length; i++) {
      var d = data[i];
      if (d.x_value >= x_minvalue && d.x_value <= x_maxvalue && d.y_value >= y_minvalue && d.y_value <= y_maxvalue) {
        global_data_overlap.push(d);
      }
    }
  }
  else {
...
var kemi_generate_grid_points_plot = function(x_grid_name, x_grid_minvalue, x_grid_maxvalue, y_grid_name, y_grid_minvalue, y_grid_maxvalue){
	...
	//the index of current selected relevant area, start from 0
	var current_area_index = +$("#pre-areas").val() - 1;
	...
	for(var i=1; i<x_num; i++){
		for(var j=1; j<y_num; j++){
			...
			*/
			...
			//check the current selected area, whether the grid point is in it
			if(grid_x_value > x_grid_range[current_area_index][0] && grid_x_value < x_grid_range[current_area_index][1] && grid_y_value > y_grid_range[current_area_index][0] && grid_y_value < y_grid_range[current_area_index][1]){
				//the grid point is within the relevant area
				point_label = "positive";
			}
			else {
...
var plot_init = function(){
    ...
    .attr("height", height + margin.top + margin.bottom);
  	...
  	$("#pre-areas").change(function() {
		//change the predicted area, so the grid points will change
		...
		if(draw_type == 0 && model_learned === true && (relevantAttributes.indexOf($("#x-attr-name").val()) > -1 || relevantAttributes.indexOf($("#y-attr-name").val()) > -1)){
			//model learned and x or y is within relevant attributes, then generate grid points and display relevant ranges
			kemi_generate_grid_points_plot(x_name, x_minvalue, x_maxvalue, y_name, y_minvalue, y_maxvalue);	
		}
    if (showingObject) {
      all_relevant_objects_btn_clicked();
    }
...
// 			}
	...
	$("#to-label-btn").on('click', function(event){
	   ...
	   $("#list_num_labeled").text(num_positive + num_negative);
	...
	$("#next-iteration-btn").on('click', function(event){
        ...
        d3.json("next_exploration.php", function(error, data) {
			...
			//generate areas in the selection
			var areas_html = "";
			for(var i=0; i<relevantAreas.length; i++){
				areas_html = areas_html + "<option value='" + (i+1) + "'>area " + (i+1) + "</option>"; 
			}
			$("#pre-areas").html(areas_html);
          ...
          if (attr_index_in_all == -1 || attr_index == -1 || (all_attr_info[attr_index_in_all].min_value == area_obj[key][0] && all_attr_info[attr_index_in_all].max_value == area_obj[key][1])) {
            ...
            if (area_obj[key][0] == area_obj[key][1]) {
              relevant_areas = relevant_areas + key + " = " + area_obj[key][1];
            }
            else {
            ...
            if (area_obj[key][0] == area_obj[key][1]) {
              relevant_areas = relevant_areas + " AND " + key + " = " + area_obj[key][1];
            }
            else {
...
$("#zoom-in-btn").on('click', function(event){
	...
	*/
    ...
    if(isNaN(zoomin_x_minvalue) || isNaN(zoomin_x_maxvalue) || isNaN(zoomin_y_minvalue) || isNaN(zoomin_y_maxvalue)){
      ...
      if (showingObject == 0) {
        kemi_scatter_plot(global_sample_data, d3.select('#chart svg'), current_x_range, current_y_range, true);
      } else {
        kemi_scatter_plot(global_objects, d3.select('#chart svg'), current_x_range, current_y_range, true);
      }
    
      if(model_learned === true && (relevantAttributes.indexOf($("#x-attr-name").val()) > -1 || relevantAttributes.indexOf($("#y-attr-name").val()) > -1)){
        //model learned and x or y is within relevant attributes, then generate grid points and display relevant ranges
        kemi_generate_grid_points_plot($("#x-attr-name").val(), zoomin_x_minvalue, zoomin_x_maxvalue, $("#y-attr-name").val(), zoomin_y_minvalue, zoomin_y_maxvalue);
  ...
  $("#goback-btn").on('click', function(event){
    ...
    if(model_learned === true && (relevantAttributes.indexOf($("#x-attr-name").val()) > -1 || relevantAttributes.indexOf($("#y-attr-name").val()) > -1)){
        //model learned and x or y is within relevant attributes, then generate grid points and display relevant ranges
        kemi_generate_grid_points_plot($("#x-attr-name").val(), +all_attr_info[x_index_to_plot].min_value, +all_attr_info[x_index_to_plot].max_value, $("#y-attr-name").val(), +all_attr_info[y_index_to_plot].min_value, +all_attr_info[y_index_to_plot].max_value);
    ...
    /*$("#all-relevant-objects-btn").on('click', function(event) {
        ...
        var relevant_areas = "";
        var area_obj = relevantAreas[current_area_index];
        var first_relevant_attr = true;
        for(var key in area_obj){
          //check if the attr is the default total range
          var attr_index = -1;
          ...
          if(first_relevant_attr == true){
            //this is the first relevant attribute
            relevant_areas = relevant_areas + area_obj[key][0] + " <= " + key + " AND ";
            relevant_areas = relevant_areas + key + " <= " + area_obj[key][1];
            first_relevant_attr = false;
          }
          else{
            //not the first relevant attribute
            relevant_areas = relevant_areas + " AND " + area_obj[key][0] + " <= " + key + " AND ";
            relevant_areas += key + " <= " + area_obj[key][1];
          }
          
        }

        predicted_query = predicted_query.split("WHERE")[0];
        predicted_query += " WHERE " + relevant_areas + ";";
...
function all_relevant_objects_btn_clicked() {
  showingObject = 1;
        global_objects = [];
        $('#tableContainer').empty();
        var current_area_index = + $("#pre-areas").val() - 1;
